export const vueVersion = '^3.3.4';
export const vueTscVersion = '^1.8.8';
export const vueRouterVersion = '^4.2.4';
export const vitePluginVueVersion = '^4.3.1';
export const vueTestUtilsVersion = '^2.4.1';
export const eslintPluginVueVersion = '^9.14.1';
export const eslintPluginPrettierVueVersion = '^7.1.0';
export const eslintPluginTypescriptVueVersion = '^11.0.3';
export const eslintPluginTypescriptVersion = '^6.18.1';
export const eslintPluginParserVersion = '^6.18.1';
export const eslintPluginPrettierReactVersion = "^9.1.0";
export const eslintPluginReactVersion = '^7.33.2';
export const postcssVersion = '8.4.21';
export const tailwindcssVersion = '3.2.7';
export const autoprefixerVersion = '10.4.13';
export const eslintVersion = '^8.56.0';
